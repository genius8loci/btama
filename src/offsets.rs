...
pub mod trap {
    ...
    /// то, что читается из XML уровня. В рантайне игра пользуется не ими,
    /// а [`POSITION`]; здесь они остаются нулевыми.
    pub const POSITION_X: usize = 0x24;
    /// `<Scale>k__BackingField` (f32) — множитель размера объекта.
    pub const SCALE: usize = 0x2C;
    ...
    /// обращении и запоминает его, а до тех пор поле остаётся нулевым.
    /// Отсюда и `0,0 0x0` у половины объектов — до них просто ещё никто не
    /// обращался. Считает игра ровно так:
    ///
    /// ```csharp
    ...
    /// ```
    ///
    /// То есть [`POSITION`] с переводом из единиц симуляции плюс размер
    /// [`SOURCE_RECT`], умноженный на [`SCALE`]. Именно это мы и повторяем,
    /// когда поле пустует.
    pub const BOUNDING: usize = 0x50;
    /// `m_Rectangle` (4 × i32) — свойство `DrawRectangle`, **кадр в атласе
    /// текстур**, а не положение в мире.
    ///
    ...
    /// но он же и есть размер объекта в мире, что видно по формуле выше.
    pub const SOURCE_RECT: usize = 0x70;
    /// `<Position>k__BackingField` (2 × f32) — положение в **единицах
    /// физического движка**, а не в пикселях.
    ///
    /// Настоящая позиция объекта: [`POSITION_X`] заполняется только при
    /// загрузке уровня из XML, а [`BOUNDING`] считается лениво из этого поля.
    /// Числа с живой игры не оставляют сомнений в том, что это за единицы:
    /// у объекта с `m_Bounding = 1488,576` здесь лежит `14.88,5.76`.
